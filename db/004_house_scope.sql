...
WITH observed AS (
  ...
  WHERE s.sold_date >= DATE '1990-01-01'
    ...
    AND p.postcode BETWEEN '6000' AND '6200'
    AND lower(trim(s.property_type)) = 'house'
  ...
  SELECT suburb_key,sum(sale_count)::bigint AS sale_count,
...
SELECT r.suburb_key,r.suburb,r.postcode AS canonical_postcode,t.observed_postcodes,t.sale_count,
  round(r.sale_count::numeric/nullif(t.sale_count,0),4) AS postcode_confidence
FROM ranked r JOIN totals t USING(suburb_key)
...
SELECT d.suburb_key,d.suburb,d.canonical_postcode AS postcode,
  date_trunc('month',s.sold_date)::date AS sale_month,count(*)::bigint AS sale_count,
  percentile_cont(.5) WITHIN GROUP(ORDER BY s.price_aud) FILTER(WHERE s.price_aud IS NOT NULL)::bigint AS median_price_aud,
  avg(s.price_aud)::bigint AS average_price_aud,min(s.price_aud) AS minimum_price_aud,max(s.price_aud) AS maximum_price_aud
FROM core.sale_event s
JOIN core.property p USING(property_id)
