...
type Segment={label:string|number;sale_count:number|string;median_price_aud:number|string|null};
type Insights={summary:{sale_count:number|string;priced_sales:number|string;land_sample:number|string;land_price_correlation:number|string|null;median_land_size_sqm:number|string|null};property_types:Segment[];bedrooms:Segment[]};
...
export default function SalesExplorer(){
  ...
  return <section className="explorer" aria-label="Property sales explorer">
    ...
    {(rows.length||loading||error)&&<div className="results" ref={resultsRef}>
      ...
      {insights&&<section className="deeper-read"><header><p className="eyebrow">WHAT SHAPES THE RESULT?</p><h3>Look beneath the median.</h3></header><div className="insight-grid"><article><span>Land and sale value</span><strong>{Number(insights.summary.land_sample)>=10?`${relationship} ${correlation>=0?"positive":"negative"} relationship`:"Not enough evidence"}</strong><small>{Number(insights.summary.land_sample)>=10?`Correlation ${correlation.toFixed(2)} across ${number.format(Number(insights.summary.land_sample))} comparable sales`:"Fewer than 10 suitable land records"}</small></article><article><span>Typical recorded land</span><strong>{insights.summary.median_land_size_sqm?`${number.format(Number(insights.summary.median_land_size_sqm))} m²`:"Not available"}</strong><small>Median of plausible 50–10,000 m² records</small></article></div><SegmentBars title="What sold" rows={insights.property_types} label={r=>String(r.label)}/><SegmentBars title="Median by bedrooms" rows={insights.bedrooms} label={r=>`${r.label} bed`}/></section>}
      <div className="data-footer"><p>Indicative historical analysis, not a valuation · MotherDuck analytical engine</p><button type="button" onClick={download} disabled={!rows.length}>Download CSV</button></div>
    </div>}
  </section>;
}

function SegmentBars({title,rows,label}:{title:string;rows:Segment[];label:(row:Segment)=>string}){const max=Math.max(...rows.map(r=>Number(r.sale_count)),1);return <div className="segments"><h4>{title}</h4>{rows.map(row=><div className="segment" key={String(row.label)}><span>{label(row)}</span><div><i style={{width:`${Math.max(4,Number(row.sale_count)/max*100)}%`}}/></div><strong>{row.median_price_aud?money.format(Number(row.median_price_aud)):"—"}</strong><small>{number.format(Number(row.sale_count))} sales</small></div>)}</div>}
